...
public class MatrixOverQueries {
  public int solution(int[][] query, int[] array){
      Map<Integer, List<int[]>>mapping = new HashMap<>();
      if(query.length == 0 || query == null) return 0;
      int res = 0;

      for(int i=0; i<query.length; i++){
          int[] distance = {query[i][0], query[i][1]};
          int val = query[i][2];
  ...
  public static void main(String[] args){
      int[][] query = {{1,3,1}, {2,4,2}, {0,3,1}};
      MatrixOverQueries mq = new MatrixOverQueries();
      int[] array = {1,1,2,3,2};
      System.out.print("res: ");
      System.out.print(mq.solution(query, array));
  }
}